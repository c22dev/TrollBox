...
class PasscodeKeyFaceManager {
    ...
    static func setFace(_ image: UIImage, for n: Int, keySize: CGFloat, customX: CGFloat, customY: CGFloat) throws {
        ...
        guard let png = newImage?.pngData() else { throw NSError(domain: "CarrierName", code: 0, userInfo: [NSLocalizedDescriptionKey: "Could not find carrier name"]) }
    ...
    static func exportFaceTheme() throws -> URL? {
        ...
        if let archiveURL = archiveURL {
            ...
            throw NSError(domain: "export", code: 0, userInfo: [NSLocalizedDescriptionKey: "error"])
        }
    }
    ...
    static func getURL(for n: Int) throws -> URL { // O(n^2), but works
        ...
        throw NSError(domain: "passface", code: 0, userInfo: [NSLocalizedDescriptionKey: "Could not found it"])
    }
    ...
    static func telephonyUIURL() throws -> URL {
        guard let url = try FileManager.default.contentsOfDirectory(at: URL(fileURLWithPath: "/var/mobile/Library/Caches/"), includingPropertiesForKeys: nil)
            .first(where: { url in url.lastPathComponent.contains("TelephonyUI") }) else { throw NSError(domain: "NSUI", code: 0, userInfo: [NSLocalizedDescriptionKey: "Could not find telephone folder"]) }
                   return url
    }
