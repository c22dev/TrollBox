                ...
                Section(header: Text("Remove some restrictions on you're device"), footer: Text("This will allow you to remove region restrictions on you're device, enabling services such as FaceTime. This is not working for everyone.")) {
                    ...
                    .onChange(of: enabled) { isEnabled in
                        do {
                            ...
                            guard var plist = try PropertyListSerialization.propertyList(from: data, format: nil) as? [String:Any] else { throw NSError(domain: "plist", code: 0, userInfo: [NSLocalizedDescriptionKey: "Could not read mobilegestalt"]) }
                            ...
                            if !FileManager.default.fileExists(atPath: origDeviceTypeURL.path) {
                                guard let currentType = ((plist["CacheExtra"] as? [String: Any] ?? [:])["oPeik/9e8lQWMszEjbPzng"] as? [String: Any] ?? [:])["ArtworkDeviceSubType"] as? Int else { throw NSError(domain: "subtype ?", code: 0, userInfo: [NSLocalizedDescriptionKey: "Could not get current device subtype"]) }
                                origDeviceType = currentType
                                remLog(origDeviceType)
                                guard let backupData = String(currentType).data(using: .utf8) else { throw NSError(domain: "convert", code: 0, userInfo: [NSLocalizedDescriptionKey: "Could not convert to data"]) }
                                try backupData.write(to: origDeviceTypeURL)
                            } else {
                                guard let data = try? Data(contentsOf: origDeviceTypeURL), let deviceTypeStr = String(data: data, encoding: .utf8), let deviceType = Int(deviceTypeStr) else { throw NSError(domain: "original", code: 0, userInfo: [NSLocalizedDescriptionKey: "Could not revert"]) }
