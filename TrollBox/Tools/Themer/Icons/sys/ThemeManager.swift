...
//            remLog(str)
    ...
    func icon(forAppID appID: String, from theme: Theme) throws -> UIImage {
        guard let image = UIImage(contentsOfFile: theme.url.appendingPathComponent(appID).path + ".png") else { throw NSError(domain: "image", code: 0, userInfo: [NSLocalizedDescriptionKey: "Could not open image"]) }
        return image
    }
