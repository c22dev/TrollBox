...
class CatalogThemeManager {
    ...
    func restoreCatalogs(progress: (String) -> ()) throws {
        guard let apps = LSApplicationWorkspace.default().allApplications() else { throw NSError(domain: "apps", code: 0, userInfo: [NSLocalizedDescriptionKey: "Could not get apps"]) }
...
//                    remLog("failed to edit rendition: \(error) \(rend.type) \(rend.name) \(rend.namedLookup)")
    ...
    private func backupAssetsURL(appURL: URL) throws -> URL {
        ...
        guard let infoPlistData = try? Data(contentsOf: appURL.appendingPathComponent("Info.plist")), let plist = try? PropertyListSerialization.propertyList(from: infoPlistData, format: nil) as? [String:Any] else { throw NSError(domain: "plist", code: 0, userInfo: [NSLocalizedDescriptionKey: "Could not read the plist"]) }
        guard let appShortVersion = (plist["CFBundleShortVersionString"] as? String) ?? plist["CFBundleVersion"] as? String else { throw NSError(domain: "oops", code: 0, userInfo: [NSLocalizedDescriptionKey: "Could not edit an asset.car, looks like the roothelper isn't working"]) }
