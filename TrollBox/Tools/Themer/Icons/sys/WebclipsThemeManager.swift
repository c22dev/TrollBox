...
class WebclipsThemeManager {
    ...
    func applyChanges(_ changes: [ThemeManager.SystemAppIconChange], progress: (Double) -> ()) throws {
        ...
        guard changesCount > 0 else { throw NSError(domain: "no changes", code: 0, userInfo: [NSLocalizedDescriptionKey: "no changes"]) }
...
//    }
    ...
    func addWebClip(bundleID: String, displayName: String) throws {
        ...
        guard var plist = try PropertyListSerialization.propertyList(from: data, format: nil) as? [String:Any] else { throw NSError(domain: "plist", code: 0, userInfo: [NSLocalizedDescriptionKey: "could not read temp plist, is it missing ?"])}
